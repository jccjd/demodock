...
async def main() -> None:
    ...
    async with sandbox:
        ...
        await _print_execution_logs(xvfb_exec)
        ...
        await _print_execution_logs(xfce_exec)
        ...
        await _print_execution_logs(vnc_exec)
        ...
        await _print_execution_logs(novnc_exec)
        ...
        # Install wget, curl, sudo, and Node.js since they're not pre-installed in the desktop image
        packages_install_exec = await sandbox.commands.run(
            "apt-get update && apt-get install -y curl sudo nodejs npm"
        )
        await _print_execution_logs(packages_install_exec)

        print("\nInstalling iFlow CLI...")
        # Install iFlow CLI (now that Node.js is available)
        install_exec = await sandbox.commands.run(
            "npm install -g @iflow-ai/iflow-cli@latest"
        )
        await _print_execution_logs(install_exec)

        print("\nRunning iFlow CLI command...")
        ...
        print("\nKeeping sandbox alive for 2 minutes to allow VNC access. Press Ctrl+C to exit sooner.")
        try:
